# Gemelo Digital — Proyecto Digital Twins

> Simulador web en tiempo real del comportamiento de lineales de riego con guiado GPS.  
> Desarrollado en Python + Streamlit. Primera implementación: **Lineal FSS**.

---

## Por qué existe este proyecto

La caja de guiado GPS que desarrolla Proxima Systems necesita recibir coordenadas y devolver órdenes de ralentización en tiempo real. Probar ese algoritmo directamente en campo supone tener el lineal montado, el hardware listo y condiciones de operación reales — costoso y lento.
...
## Cómo funciona — la física de la simulación
...
El FSS mantiene sus dos torres flanqueantes alineadas rígidamente con el eje Cart–End-tower tras cada tick de simulación: no importa cuánto se desvíen por la física, el FSS las recoroca.

```
CART  — I1 — I2 — [FSS_izq — FSS_der★] — I4 — I5 — END
  ↑ duty    ↑ sigue diagonal                       ↑ duty
  cycle     (×1.5)           (★ ×2.0 = motor rápido)
```

### Duty cycle (ciclo de encendido/apagado)
...
### Modo slow_down — cómo el guiado produce los giros
...
### `componentes.py` — Los bloques de construcción

Contiene las clases base reutilizables para cualquier máquina que se quiera simular:

| Clase | Qué representa |
|---|---|
| `TramoFinal` | Torre extrema (Cart o End-tower). Avanza por duty cycle. Soporta modo slow_down. |
...
- **`start() / stop()`** — arranca o para la simulación (stop también resetea los flags slow_down).
...
### `ui/panel.py` — El fragmento en tiempo real
...
> **`[IMAGEN: Captura completa de la interfaz — sidebar izquierdo con configuración y panel principal con mapa y métricas]`**
...
### Paso 2 — Configurar la velocidad (Panel speed)
...
| Lineal loco | Patinaje máximo. Para ver cómo se comporta el guiado bajo condiciones extremas. |

Se muestra una estimación de la deriva acumulada al final del campo para orientar la elección.

---

### Paso 4 — Auto-reverse (opcional)
...
### Paso 5 — Conexión externa (opcional)
...
> **`[IMAGEN: Sidebar con la simulación en marcha — parámetros bloqueados y botón STOP visible]`**

---

### El panel principal en detalle

> **`[IMAGEN: Panel principal durante simulación — barra de progreso + logs + métricas + mapa]`**
...
> **`[IMAGEN: Detalle del mapa Plotly — torres con anotaciones, tramo FSS más ancho, colores de alineación]`**
...
Cada tramo se dibuja como una viga con celosía interior. El color indica el estado de alineación:

| Color | Estado |
|---|---|
| Verde | Alineado — dentro de tolerancia (< 5 cm de desviación). |
| Amarillo | Alineado con ligera desviación angular. |
| Rojo | Desalineado — el motor de la torre intermedia está corrigiendo. |

El tramo FSS se dibuja más ancho que el resto para reflejar su mayor rigidez estructural. Su anotación indica `FSS` en lugar del ángulo de desviación.
...
1. Dibuja la trayectoria como línea discontinua en el mapa.
...
> **`[IMAGEN: Detalle del panel con trayectoria activa — línea discontinua en el mapa y chips de error Δd / Δrumbo]`**
...
```bash
pip install streamlit plotly pyserial
```

### Arrancar la aplicación

Desde la raíz del repositorio (`pivot_lineal/`):
...
*Desarrollado por Proxima Systems.*
